...
public class PDFGeneratorServiceImpl implements PDFGeneratorService {
    ...
    private static final String ERROR_PARSING_PDF = "Error while parsing PDF template: ";
    private static final String ERROR_READING_TEMPLATE = "Error while reading the template file %s";
    ...
    public byte[] generatePostpay(long shipmentId) {
        ...
        try {
            ...
            log.error(ERROR_PARSING_PDF + e.getMessage());
        } catch (NullPointerException e) {
            log.error(ERROR_READING_TEMPLATE, PDF_LABEL_TEMPLATE);
        }
        return data;
    }
    ...
    public byte[] generateLabel(long shipmentId) {
        ...
        try {
            ...
            if (acroForm != null) {
                ...
                double weightSum = shipment.getParcelList().stream().mapToDouble(Parcel::getWeight).sum();
                field.setValue(String.valueOf(weightSum));

                field = (PDTextField) acroForm.getField("value");
                BigDecimal declaredPriceSum = shipment.getParcelList().stream()
                        .map(Parcel::getDeclaredPrice).reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
                field.setValue(String.valueOf(declaredPriceSum));
